...
function validateForPublish(data) {
  ...
  if (!data.deadline) {
    ...
    const MIN_DURATION_MS = 1 * 60 * 1000; // 5 minute
    ...
    if (isNaN(d.getTime())) {
      ...
      errors.deadline =
        "Deadline-ul trebuie să fie cel puțin 5 minute în viitor.";
      if (!missing.includes("deadline")) missing.push("deadline");
    } else if (d.getTime() > now + MAX_DURATION_MS) {
      errors.deadline =
        "Deadline-ul nu poate fi mai departe de 1 an în viitor.";
      if (!missing.includes("deadline")) missing.push("deadline");
    }
  }
...
function pickAuctionFields(body = {}) {
  ...
  if (body.startPrice !== undefined)
    out.startPrice =
      body.startPrice === "" || body.startPrice === null
        ? undefined
        : Number(body.startPrice);
  if (body.targetPrice !== undefined)
    out.targetPrice =
      body.targetPrice === "" || body.targetPrice === null
        ? null
...
router.get("/:id", optionalAuth, validateObjectId("id"), async (req, res) => {
  try {
    const auction = await Auction.findById(req.params.id).populate(
      "buyer",
      "firstName lastName companyName email phone avatar rating",
    );

    if (!auction) {
      logger
        .fromReq(req)
        .warn(EVENTS.AUCTION.NOT_FOUND, "Licitație inexistentă accesată", {
          entityType: "auction",
    ...
    if (auction.status === "draft") {
      const uid = req.user?.id || null;
      const ownerId =
        auction.buyer?._id?.toString() || auction.buyer?.toString();
      ...
      if (!isOwner && !isAdmin) {
        logger
          .fromReq(req)
          .security(
            EVENTS.AUCTION.DRAFT_ACCESS_DENIED,
...
router.post("/", authMiddleware, auctionCreateLimiter, async (req, res) => {
  try {
    if (req.user.role !== "buyer") {
      ...
      return res
        .status(403)
        .json({ message: "Doar cumpărătorii pot crea licitații" });
    }
    ...
    if (wantsPublish) {
      ...
      if (!valid) {
        logger
          .fromReq(req)
          .warn(
            EVENTS.AUCTION.PUBLISH_VALIDATION_FAIL,
...
router.put("/:id", authMiddleware, validateObjectId("id"), async (req, res) => {
  try {
    ...
    if (auction.status === "closed" || auction.status === "cancelled") {
      return res
        .status(400)
        .json({ message: "Licitația nu mai poate fi editată" });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (auction.status !== "draft") {
        return res
          .status(400)
          .json({ message: "Doar drafturile pot fi publicate." });
      }
      ...
      if (!valid) {
        ...
        return res.status(400).json({
          message:
            "Completează toate câmpurile obligatorii înainte de publicare.",
          missing,
          errors,
        });
      }
      ...
      logger
        .fromReq(req)
        .audit(
          EVENTS.AUCTION.DRAFT_PUBLISHED,
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      if (
        auction.buyer.toString() !== req.user.id &&
        req.user.role !== "admin"
      ) {
        logger
          .fromReq(req)
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!content?.trim())
        return res.status(400).json({ message: "Mesajul e gol" });

      const auction = await Auction.findById(req.params.id);
      if (!auction)
        return res.status(404).json({ message: "Licitația nu există" });
      if (auction.status !== "active")
        return res.status(400).json({ message: "Licitația nu e activă" });
      ...
      if (!isBuyer && !hasBid) {
        ...
        return res.status(403).json({
          message:
            "Doar cumpărătorul și furnizorii care au ofertat pot scrie aici",
        });
      }
