...
function formatFieldValue(field, val) {
  ...
  if (field === "location" && typeof val === "object")
    return val.address || val.city || JSON.stringify(val);
  return String(val);
}
    ...
    <div>
      {bidCount > 0 && (
        ...
        >
          ...
          <span
            style={{ fontSize: "0.8125rem", fontWeight: 600, color: "#92400E" }}
          >
            Licitația are {bidCount}{" "}
            {bidCount === 1 ? "ofertă existentă" : "oferte existente"} —
            verifică impactul modificărilor
          </span>
        </div>
      )}
      <table
        ...
        <tbody>
          {changedFields.map((field) => {
            ...
            return (
              <tr
                key={field}
                style={{ background: isImportant ? "#FFFBEB" : "transparent" }}
              >
                <td
                  style={{
                    padding: "7px 8px",
                    fontWeight: 600,
                    color: isImportant ? "#92400E" : "var(--text-heading)",
                    whiteSpace: "nowrap",
                  }}
                >
                  {isImportant && "⚠️ "}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Cerere de{" "}
          <strong>{request.type === "edit" ? "editare" : "ștergere"}</strong>{" "}
          pentru <strong>"{request.auction?.title}"</strong>
        </p>
        <div className="form-group">
          ...
          <textarea
            ...
            placeholder="Explică buyerului de ce respecți cererea..."
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: 16 }}>
          <p style={{ fontSize: "0.8125rem", margin: "0 0 4px" }}>
            <strong>Buyer:</strong> {request.buyer?.firstName}{" "}
            {request.buyer?.lastName} ({request.buyer?.email})
          </p>
          {request.reason && (
            <p style={{ fontSize: "0.8125rem", margin: "0 0 4px" }}>
              <strong>Motivul buyerului:</strong> {request.reason}
            </p>
          )}
          {request.adminNote && (
            <p style={{ fontSize: "0.8125rem", margin: "0 0 4px" }}>
          ...
          {request.reviewedBy && (
            <p style={{ fontSize: "0.8125rem", margin: "0 0 4px" }}>
              <strong>Recenzat de:</strong> {request.reviewedBy?.firstName}{" "}
              {request.reviewedBy?.lastName} la{" "}
        ...
        {request.type === "delete" && request.bidCount > 0 && (
          ...
          >
            ...
            >
              ⚠️ Licitația are {request.bidCount}{" "}
              {request.bidCount === 1 ? "ofertă existentă" : "oferte existente"}
              . Ștergerea va fi soft (status = cancelled) — datele se păstrează.
            </span>
          </div>
        )}
      </div>
...
export default function AdminDashboard() {
  ...
  const downloadAdminInvoice = async (inv) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/invoices/auction/${inv.auction._id}/download`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      if (!res.ok) return;
      ...
      a.href = url;
      a.download = `RevBid-${inv.invoiceNumber}.pdf`;
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/auction-requests?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const approveRequest = async (requestId) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/auction-requests/${requestId}/approve`,
  ...
  const rejectRequest = async (requestId, adminNote) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/auction-requests/${requestId}/reject`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (res.ok) {
        setRequests((prev) =>
          prev.map((r) =>
            r._id === requestId ? { ...r, status: "rejected", adminNote } : r,
          ),
        );
        setRejectModal(null);
      }
  ...
  const roleBadge = (r) =>
    ({ buyer: "badge-blue", supplier: "badge-teal", admin: "badge-navy" })[r] ||
    "badge-gray";
  const statusBadge = (s) =>
    ({
      active: "badge-solid-teal",
      closed: "badge-solid-gray",
      cancelled: "badge-solid-red",
      draft: "badge-solid-amber",
    })[s] || "badge-solid-gray";
      ...
      <div className="container">
        ...
        <div className="stats-grid stats-grid-4">
          <div className="stat-card">
            <p className="stat-value" style={{ color: "var(--action-blue)" }}>
              {users.length}
            </p>
            <p className="stat-label">Total useri</p>
          </div>
          <div className="stat-card">
            <p className="stat-value" style={{ color: "var(--error-red)" }}>
              {users.filter((u) => u.isBanned).length}
            </p>
            <p className="stat-label">Useri banați</p>
          </div>
          <div className="stat-card">
            <p className="stat-value" style={{ color: "var(--bid-teal)" }}>
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u._id}>
                    ...
                    <td>
                      <span
                        className={`badge ${u.isBanned ? "badge-solid-red" : "badge-solid-green"}`}
                      >
                        {u.isBanned ? "Suspendat" : "Activ"}
                      </span>
                    </td>
                    <td>
                      <button
              ...
              <tbody>
                {auctions.map((a) => (
                  <tr key={a._id}>
                    ...
                    <td>
                      {a.buyer?.firstName} {a.buyer?.lastName}
                    </td>
                    <td style={{ fontWeight: 600, color: "var(--bid-teal)" }}>
                      {a.currentPrice} RON
                    </td>
                    <td>
                    ...
                    style={{
                      ...
                      background:
                        reqTypeFilter === f
                          ? "var(--primary-navy)"
                          : "var(--bg-card)",
                      color: reqTypeFilter === f ? "#fff" : "var(--text-body)",
                      ...
                      style={{
                        ...
                        background:
                          reqStatusFilter === f
                            ...
                            : "var(--bg-card)",
                        color:
                          reqStatusFilter === f ? "#fff" : "var(--text-body)",
                        transition: "all 0.15s",
                      }}
                    >
                      {f === "all"
            ...
            {requests.length === 0 ? (
              <div className="empty-state">
                ...
                <p className="empty-state-text">
                  Nu există cereri cu filtrele selectate.
                </p>
              </div>
            ) : (
              <div className="table-wrap">
        ...
        {!loading &&
          ...
          (invoices.length === 0 ? (
            <div className="empty-state">
              ...
              <p className="empty-state-text">
                Rezumatele de tranzacție apar automat când licitațiile cu
                câștigător se finalizează.
              </p>
            </div>
          ) : (
            <div className="table-wrap">
                ...
                <tbody>
                  {invoices.map((inv) => (
                    <tr key={inv._id}>
                      ...
                      >
                        {inv.auction?.title || inv.auctionTitle || "—"}
                      </td>
                      <td>
                        {inv.buyer
                          ? `${inv.buyer.firstName} ${inv.buyer.lastName}`
                          : inv.buyerName || "—"}
                      </td>
                      <td>
                        {inv.supplier
                          ? `${inv.supplier.firstName} ${inv.supplier.lastName}`
                          : inv.supplierName || "—"}
                      </td>
                      <td style={{ fontWeight: 700, color: "var(--bid-teal)" }}>
                        {Number(inv.amount).toLocaleString("ro-RO")}{" "}
                        {inv.currency}
                      </td>
                      <td>
                        <span className="badge badge-navy">{inv.status}</span>
                      </td>
                      <td style={{ fontSize: "0.75rem", whiteSpace: "nowrap" }}>
                        <span title="Email cumpărător">
                          {inv.emailedAtBuyer ? "✅" : "—"} B
                        </span>
                        {"  "}
                        <span title="Email furnizor">
                          {inv.emailedAtSupplier ? "✅" : "—"} F
                        </span>
                      </td>
                      <td>
                        {inv.auction?._id && (
                          <button
                            className="btn btn-sm btn-outline"
                            onClick={() => downloadAdminInvoice(inv)}
                          >
                            ⬇ PDF
                          </button>
                        )}
