...
export default function Settings() {
  ...
  const handleAvatarUpload = async e => {
    ...
    try {
      ...
      const data = await res.json(); if (!res.ok) { setError(data.message); return; }
      login(data, token); setMsg('Poza de profil actualizată!');
  ...
  const handleChangePassword = async e => {
    ...
    if (passwords.newPassword.length < 6) { setError('Parola nouă trebuie să aibă minim 6 caractere'); return; }
    setLoading(true);
    try {
      ...
      const data = await res.json(); if (!res.ok) { setError(data.message); return; }
      setMsg('Parola schimbată cu succes!'); setPasswords({ currentPassword: '', newPassword: '', confirmPassword: '' });
    } catch { setError('Eroare de conexiune'); } finally { setLoading(false); }
  };

  const handleDeleteAccount = async () => {
    try { await fetch(`${API_URL}/api/auth/account`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } }); logout(); navigate('/login'); }
    catch { setError('Eroare la ștergerea contului'); }
  };
  ...
  return (
    <div className="page">
      <div className="container-sm">
        <h1 className="page-title" style={{ marginBottom: '1.5rem' }}>⚙️ Setări cont</h1>
          ...
          <div className="card">
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Click pe poză pentru a o schimba</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSaveProfile} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              <div className="form-group"><label className="form-label">Oraș</label><input className="form-input" name="city" placeholder="ex: București" value={form.city} onChange={handleChange} /></div>
              {user?.role === 'supplier' && (
        ...
        {tab === 'security' && (
          <div className="card">
            <h3 className="card-title" style={{ marginBottom: '1rem' }}>Schimbă parola</h3>
            {!user?.passwordHash && user?.googleId ? (
              <div className="alert alert-info">Contul tău este autentificat prin Google — nu ai o parolă setată.</div>
            ) : (
              <form onSubmit={handleChangePassword} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <div className="form-group"><label className="form-label">Parola curentă</label><input className="form-input" type="password" name="currentPassword" value={passwords.currentPassword} onChange={handlePasswordChange} required /></div>
                <div className="form-group"><label className="form-label">Parola nouă</label><input className="form-input" type="password" name="newPassword" value={passwords.newPassword} onChange={handlePasswordChange} required /></div>
                <div className="form-group"><label className="form-label">Confirmă parola nouă</label><input className="form-input" type="password" name="confirmPassword" value={passwords.confirmPassword} onChange={handlePasswordChange} required /></div>
                <button className="btn btn-primary btn-block" type="submit" disabled={loading}>{loading ? 'Se schimbă...' : 'Schimbă parola'}</button>
              </form>
            )}
          </div>
        )}
        ...
        {tab === 'danger' && (
          <div className="card">
            <h3 className="card-title" style={{ marginBottom: '1rem' }}>Zonă periculoasă</h3>
            <div className="alert alert-warning" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
              <div>
                <p style={{ fontWeight: 600, color: '#92400E', margin: '0 0 4px' }}>Șterge contul</p>
                <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', margin: 0 }}>Această acțiune este ireversibilă. Toate datele vor fi șterse.</p>
              </div>
              <button className="btn btn-danger btn-sm" onClick={() => setShowDeleteConfirm(true)}>Șterge contul</button>
            </div>
            {showDeleteConfirm && (
              <div className="alert alert-error" style={{ marginTop: '1rem' }}>
                <div style={{ width: '100%' }}>
                  <p style={{ fontWeight: 600, marginBottom: '10px' }}>Ești sigur? Această acțiune <strong>nu poate fi anulată</strong>.</p>
                  <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                    <button className="btn btn-outline btn-sm" onClick={() => setShowDeleteConfirm(false)}>Anulează</button>
                    <button className="btn btn-danger btn-sm" onClick={handleDeleteAccount}>Da, șterge contul meu</button>
                  </div>
                </div>
              </div>
            )}
